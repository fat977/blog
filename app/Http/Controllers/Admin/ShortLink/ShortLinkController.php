...
class ShortLinkController extends Controller
{
    ...
    public function store(ShortLinkValidation $request)
    {
        ...
        return redirect()->route('admin.short_links.index')->with('success', __('admin/shortlink/shortlink.messages.create'));
    }
    ...
    public function update(ShortLinkValidation $request, $id)
    {
        ...
        return redirect()->route('admin.short_links.index')->with('success', __('admin/shortlink/shortlink.messages.edit'));
    }
    ...
    public function destroy(string $id)
    {
        ...
        return redirect()->back()->with(['success' =>  __('admin/shortlink/shortlink.messages.delete')]);
    }
    ...
    public function statistics($short_link_id)
    {
        ...
        if (count($browserVisits) < 1) {
            session()->flash('error', __('admin/shortlink/shortlink.messages.no_browsers'));
        }

        if (count($countryVisits) < 1) {
            session()->flash('error', __('admin/shortlink/shortlink.messages.no_countries'));
        }

        if (count($countryVisits) < 1 && count($browserVisits) < 1) {
            session()->flash('error', __('admin/shortlink/shortlink.messages.no_both'));
        }
