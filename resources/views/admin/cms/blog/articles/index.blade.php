        ...
        <table class="table table-striped text-center" id="articles">
            <thead>
                <tr>
                    ...
                    <th>{{ __('admin/cms/blog/article.fields.comments') }} </th>
            ...
            <tbody>
                ...
                <tr>
                    <td><a target="_blank" href="{{ route('guest.articles.show', $article->slug) }}">{{ $article->title }}</a></td>
                    <td>

                        @if (!$article->image)
                        <img src="{{ $article->image_default }}" alt="{{ $article->title }}" width="70px" height="70px">
                        @else
                        <img src="{{ asset('storage/articles/' . $article->image) }}" alt="{{ $article->title }}" width="70px" height="70px">
                        @endif
                    </td>
                    <td>
                    ...
                    <td>
                        <img src="{{ $article->user->avatar_image }}" style="width:50px; height:50px" class="rounded circle"><br>
                        {{ $article->user->name }}
                    </td>

                    <td>
                        <a target="_blank" href="{{ route('admin.comments.index', $article->id) }}">{{ $article->comments->count()}}</a>
                    </td>
                    <td>
                        <a href="{{ route('guest.articles.show', $article->slug) }}" class="mx-1 btn btn-info"><i class="fas fa-eye"></i></a>
                        <a href="{{ route('admin.articles.edit', $article->id) }}" class="mx-1 btn btn-success"><i class="fas fa-edit"></i></a>
                        <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#confirm-delete-{{ $article->id }}">
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer justify-content-between">
                                        <button type="button" class="btn btn-default btn-md" data-dismiss="modal">{{ __('admin/cms/blog/article.extra.close') }}</button>
